"""UnoArena analytics-api — placeholder service (canned responses only, no real logic).
...
def route(method: str, path: str) -> tuple[int, dict[str, Any]]:
    ...
    Canned responses only — no persistence, no real analytics logic.
    """
    if method == "GET" and path == "/health":
        return 200, {"status": "ok", "service": SERVICE}
    if method == "GET" and path == "/tournaments/sample/bracket":
        return 200, {"bracket": []}
