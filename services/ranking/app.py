"""UnoArena ranking — placeholder service (canned responses only, no real logic).
...
def route(method: str, path: str) -> tuple[int, dict[str, Any]]:
    ...
    Canned responses only — no persistence, no real ranking logic.
    """
    if method == "GET" and path == "/health":
        return 200, {"status": "ok", "service": SERVICE}
    if method == "GET" and path == "/players/sample/rating":
        return 200, {"elo": 1000}
