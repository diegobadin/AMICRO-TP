...
| `RoomCreationFailed` | Room | A `RoomCreationRequested` (tournament) could not be honored (invalid player count, persistent error) | `roomId`, `tournamentId`, `roundNumber`, `reason` | Tournament Orch. (retry/compensate/alert — see §6.4.3, §7.4.2) |
| `RoomExpired` | Room | A room in `Waiting` exceeded the inactivity timeout with no game started | `roomId`, `roomType`, `reason: waiting_timeout` | Tournament Orch. (treats as all-forfeit room — see §6.8.5, §7.4.2), Analytics |
| `StaleCommandRejected` | Room | State-mutating request with stale sequence number | `roomId`, `playerId`, `expectedSeq`, `receivedSeq` | (Client via conditional-request rejection — stale seq → `412`, see Architecture §2.3.1; not propagated to other contexts) |
