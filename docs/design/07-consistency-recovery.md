...
Room Aggregate checks: room.seq == N ?
  ...
  │
  └── No  → Reject as a conflict (force reconciliation)
            Return { currentSeq: room.seq, latestState }
            Client reconciles and retries
```

> **HTTP wire mapping (normative for the whole package).** Throughout these design documents a stale state-mutating command is described at the domain abstraction as a *conflict rejection* (historically written "409"). The architecture refines the wire contract (Architecture §2.3.1): the sequence number is the resource `ETag`; a stale `If-Match` ⇒ **`412 Precondition Failed`**, a missing `If-Match` ⇒ **`428 Precondition Required`**, and **`409 Conflict`** is reserved for non-sequence state conflicts (act-out-of-turn, join a started/full room, start a game already in progress). The domain invariant — reject the stale command and force the client to reconcile — is unchanged by this mapping.
