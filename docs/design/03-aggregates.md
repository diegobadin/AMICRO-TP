...
1. **Sequence serialization:** Every command must carry the expected `sequenceNumber`. If `command.seq ≠ room.seq`, reject as a conflict (HTTP wire mapping in Architecture §2.3.1: stale sequence → `412 Precondition Failed`).
