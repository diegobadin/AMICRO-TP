...
| **Sequence Number** | A monotonically increasing integer attached to every state-mutating command accepted by a Room. Used to enforce strict serialization and reject stale commands as a conflict (HTTP wire mapping defined in Architecture §2.3.1: stale sequence → `412 Precondition Failed`). |
