...
| **D5** | **The Grafana admin credential is sealed into `monitoring` via a new `gitops/secrets/platform/` path, synced by its own Application in the platform chart.** | `gitops/secrets/<env>` is synced by the `unoarena` project into `unoarena-<env>`, and that project's destinations are `argocd` + `unoarena-*` — it cannot deliver into `monitoring`. `seal.sh` already takes `-n <namespace>`, so the sealing side is one more `seal` call and one more generated password. |
