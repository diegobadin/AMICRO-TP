...
| E2 | What an SSE frame carries | **A public event envelope with `id` = `sequenceNumber`.** The CLI applies it to the board and re-reads `GET /rooms/{id}/games/{n}` only when the event changed its own hand or a seq gap appeared. Hands never enter the fan-out tier — the boundary P6's spectator work depends on |
