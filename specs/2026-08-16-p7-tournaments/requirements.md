...
|---|---|---|
| **D1** | The internal routes are protected by a **sealed shared token** (`X-Internal-Token`), required across the whole `/internal` prefix. **No NetworkPolicy manifest is shipped.** | The architecture's stated control for `/internal/` is "not routed through the gateway + NetworkPolicy" (T4, D6). But **kind runs kindnet, which does not enforce NetworkPolicy**, and the EKS VPC CNI does not either without extra setup — so a policy would be enforced in *no* environment this project runs, which is the dead-manifest version of the defect D4 avoided by not creating an unwritten bracket table. Worse, it would become load-bearing the first time a cluster *did* enforce it, untested, at the exam. The token works everywhere. **Refined during F2** (was: "ship the policy too"); the reasoning belongs in the threat model and the presentation, not in YAML nobody applies. |
| **D1b** | The token covers the **existing** tick route too, not only the new one — so `timer-worker` sends it as well. | F2 found that `SYSTEM_AUTH` was never a credential: it checks `X-Player-Id` starts with `system:`, which any pod in the cluster can spell. One prefix, two rules would be the inconsistency a reviewer flags; and the route that now creates rooms sits behind the same door as the one that ticks a clock. An unset token **closes** the routes rather than opening them. |
