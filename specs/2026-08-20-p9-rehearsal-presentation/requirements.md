...
| **D3′** | **Mechanism refined during implementation: ktlint runs against a generated baseline, and the existing code is NOT reformatted.** | D3 assumed "auto-fixable" meant cheap. Measured: `ktlintFormat` moved **5,870 lines across 64 files** — every main source file of both services, not just tests — and still failed on 2 `no-consecutive-comments` violations, a rule that collides with this repo's deliberate comment density. A detour through `ktlint_code_style = intellij_idea` was tried on the theory that the code was written to IntelliJ defaults and made it **worse** (1,060 violations vs 107), so the default style stayed. A 5,870-line no-op diff on the two services the demo leans on hardest is exactly the churn P9's own R1 warns about. The baseline (1,377 entries) keeps `check` honest going forward — bite-checked: a new violation fails, existing code passes — and lists the debt instead of hiding it. |
...
| **D5′** | **Mechanism refined: the mirror job is gated on `changes: [ci/mirror-bases.yml]` rather than `when: manual`.** | A permanently-available manual button would add a second manual job to every pipeline and change the "43 jobs, 42 success + 1 manual" shape the closure runs are read by. The changes-gate is also better semantics — the job re-runs exactly when you edit what it mirrors, which is what refreshing a base means. Verified: the push carrying it produced a one-job pipeline, and the next push ran all ten services with no mirror job. |
| **D7** | **The two Kotlin services set `JAVA_TOOL_OPTIONS=-XX:MaxRAMPercentage=75` in the same template as their memory limit.** | Not anticipated by D4. A JVM sizes its heap *from* the container limit and defaults to 25% of it, so adding the limit alone would have cut room-gameplay's heap to 192 MiB — below its measured 307 MiB peak. A memory limit on a JVM container is an input to the runtime, not a config value, and the two must move together or the limit is a latent OOMKill. |
| **D8** | **The CLI accepts `Client-Checkpoint.md` §5's canonical positional forms.** | Not a new command (which is out of scope) — the documented surface simply did not work. `bot --tournament <id>`, the faculty's own invocation, silently played a *casual* game, and `tournament status <id>` silently reported on a *different* tournament. §8 evaluates contract conformance explicitly. Additive: the `--id`/`--room` spellings still work. |
